define("three-canvas", [
  ...
  "underscore",
  "three-orbit",
], function (THREE, $, utils, loadBeeb, _) {
  "use strict";
  ...
  class ClickControls {
    constructor(domElement, scene, camera, orbitControls, keyboardGroup) {
      ...
      this.domElement.addEventListener(
        "pointerdown",
        (event) => this.onDown(event),
        false
      );
      this.domElement.addEventListener(
        "pointerup",
        (event) => this.onUp(event),
        false
      );
      this.domElement.addEventListener(
        "pointermove",
    ...
    getObjectForEvent(event) {
      ...
      if (
        intersectedObjects[0].point.y > 2.6 ||
        intersectedObjects[0].object === this.downObj
      ) {
    ...
    _setDown(obj) {
      if (obj === this.downObj) return;
      if (this.downObj) {
        if (this.downObj.onUp) this.downObj.onUp();
        this.downObj = null;
      }
      this.downObj = obj;
      if (this.downObj && this.downObj.onDown) this.downObj.onDown();
    }

    onMove(event) {
      if (this.isTracking) this._setDown(this.getObjectForEvent(event));
    }
    ...
    onDown(event) {
      ...
      if (this.isTracking) this.orbitControls.enabled = false;
    }
  ...
  class ThreeCanvas {
    constructor(canvas) {
      ...
      try {
        ...
        // Create OrbitControls with restrictions
        this.controls = new THREE.OrbitControls(
          this.camera,
          this.renderer.domElement
        );
        ...
        $("#shadows-option").change(
          function () {
            if ($("#shadows-option").is(":checked")) {
        ...
        $("#dark-option").change(
          async function () {
            if ($("#dark-option").is(":checked")) {
              ...
              let bg = await this.loadBackgroundTexture(
                "./virtual-beeb/textures/equirectangular-bg-light.jpg"
              );
        ...
        $("#cub-option").click(
          function () {
            ...
            this.camera.position.set(
              screen.position.x,
              screen.position.y + 17,
              screen.position.z + 17
            );
            this.camera.lookAt(
              screen.position.x,
        ...
        $("#unpredictable-option").click(
          async function () {
            ...
            for (let i = 0; i < 100; i++) {
              ...
              this.beebs[i].position.x =
                this.beebs[i].position.x + (50 * -22 + i * 22);
    ...
    async loadBackgroundTexture(filename) {
      const bgTexture = await utils.promisifyLoad(
    ...
    async load() {
      ...
      if (this.cpu) this.beeb.setProcessor(this.cpu);
    ...
    setProcessor(cpu) {
      this.cpu = cpu;
      if (this.beeb) this.beeb.setProcessor(cpu);
    }
